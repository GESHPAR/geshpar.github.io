# -*- coding: utf-8 -*-
# rebuild_all.py — دیتابیس + sitemap + آرشیو، یکجا از روی همه فایل‌ها
import json, re
...
def main():
    files = [p for p in ROOT.rglob('*.html') if not junk(p)]
    db, urls, rows = [], [], []
    for p in files:
    ...
    # صفحه فهرست (آرشیو) — لینک همه مقاله‌ها یکجا
...
<body><main><h1>فهرست کامل مقالات ({len(rows)} مطلب)</h1><ul>{chr(10).join(rows)}</ul></main></body></html>""", encoding='utf-8')
    ...
    print(f"✅ صفحات: {len(files)} | sitemap: {len(urls)} | دیتابیس: {len(db)}")
